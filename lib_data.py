...
class Dataset(lib_util.Factory):
    ...
    def __init__(self, basepath, hparams, fold):
        ...
        Raises:
          ValueError: if requested a temporal resolution shorter then
          that available in the dataset.
        """
        ...
        # Update default pitch ranges in hparams to reflect that of dataset.
    ...
    def get_featuremaps(self, sequences=None):
        ...
        Encodes sequences into randomly cropped and masked pianorolls,
        and returns a padded Batch containing three channels: the
        pianorolls, the corresponding masks and their lengths before
        padding (but after cropping).

        Args:
          sequences: the collection of sequences to convert. If not
          given, the entire dataset is converted.
        ...
        (pianorolls, masks), lengths = lib_util.pad_and_stack(pianorolls,
                                                              masks)
...
class Batch(object):
    ...
    def __init__(self, **kwargs):
        ...
        Args:
          **kwargs: data dictionary. Must have three keys "pianorolls",
          "masks", "lengths", each corresponding to a model placeholder.
          Each value is a sequence (i.e. a batch) of examples.
        """
    ...
    def get_feed_dict(self, placeholders):
        ...
        Args:
          placeholders: placeholder dictionary. Must have three keys
          "pianorolls", "masks" and "lengths".

        Returns:
          A feed dict mapping the given placeholders to the data in
          this batch.
        """
        assert set(placeholders.keys()) == self.keys
        return dict(
            (placeholders[key], self.features[key]) for key in self.keys)

...
flags.DEFINE_string(
    'run_id', '',
    'A run_id to add to directory names to avoid accidentally overwriting'
    'when testing same setups.')
...
def estimate_popstats(unused_sv, sess, m, dataset, unused_hparams):
    ...
    for _ in range(nepochs):
        batches = (
            dataset.get_featuremaps().batches(size=m.batch_size,
                                              shuffle=True))
    ...
    for unused_j, (tfpopstat, nppopstat) in enumerate(zip(tfpopstats,
                                                          nppopstats)):
...
def run_epoch(supervisor, sess, m, dataset, hparams, eval_op, experiment_type,
    ...
    for unused_step, batch in enumerate(batches):
        ...
        fetches = [
            m.loss, m.loss_total, m.loss_mask, m.loss_unmask,
            m.reduced_mask_size, m.reduced_unmask_size, m.learning_rate,
            eval_op
        ]
        feed_dict = batch.get_feed_dict(m.placeholders)
        ...
        # Multiply the mean loss_mask by reduced_mask_size for
        # aggregation as the mask size could be different for every batch.
    ...
    tf.logging.info(
        ...
        (
            np.log(run_stats['learning_rate'])
...
def main(unused_argv):
    ...
    with tf.Graph().as_default():
        ...
        sv = tf.train.Supervisor(
            logdir=logdir,
            saver=(tf.train.Supervisor.USE_DEFAULT
                   if FLAGS.log_progress else None),
            summary_op=None,
...
class Tracker(object):
    ...
    def __call__(self, loss, sess):
        if self.sign * loss > self.sign * self.best:
            if FLAGS.log_progress:
                tf.logging.info('Previous best %s: %.4f.',

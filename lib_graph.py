...
class CoconetGraph(object):
    ...
    def get_convnet_input(self):
        ...
        if self.hparams.mask_indicates_context:
            # flip meaning of mask for convnet purposes: after flipping,
            # mask is hot where values are known. this makes more sense
            # in light of padding done by convolution operations: the
    ...
    def compute_cross_entropy(self, logits, labels):
        if self.hparams.use_softmax_loss:
            # don't use tf.nn.softmax_cross_entropy because we need the
            # shape to remain constant
    ...
    def compute_loss(self, unreduced_loss):
        ...
        mask_size = tf.reduce_sum(mask, axis=[1, variable_axis],
                                  keep_dims=True)
        unmask_size = tf.reduce_sum(unmask, axis=[1, variable_axis],
                                    keep_dims=True)
        ...
        # NOTE: indexing the pitch dimension with 0 because the mask
        # is constant across pitch. Except in the sigmoid case, but
        # then the pitch dimension will have been reduced over.
        ...
        self.loss = (
            self.loss_mask
            if self.hparams.optimize_mask_only
    ...
    def apply_batchnorm(self, x):
        ...
        decay = 0.01
        if self.is_training:
            batchmean, batchvariance = tf.nn.moments(x, [0, 1, 2],
                                                     keep_dims=True)

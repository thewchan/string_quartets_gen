...
flags.DEFINE_string('sample_npy_path', None,
                    'Path to samples to be evaluated.')
...
def main(unused_argv):
    ...
    evaluator = lib_evaluation.EnsemblingEvaluator(
        evaluator, FLAGS.ensemble_size)
...
def evaluate_fold(fold, evaluator, hparams, eval_logdir, checkpoint_dir):
    ...
    log_fname = '%s__%s.npz' % (
        os.path.basename(checkpoint_dir), eval_run_name)
...
def get_path_pianorolls(path):
    pianoroll_fpath = os.path.join(
        tf.resource_loader.get_data_files_path(), path)
